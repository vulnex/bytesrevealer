...
class SafeStructureParser {
  ...
  async parseViewport(data, _startOffset, _endOffset) {
    if (!data) {
      return []
    }
  ...
  parsePE(bytes) {
    ...
    dosHeaderFields.push(this.createField('Magic', 'MZ (0x4D5A)', 0, 2))

    if (bytes.length >= 4) {
      dosHeaderFields.push(this.createField('Bytes in Last Page', bytes[2] | (bytes[3] << 8), 2, 2))
    }

    if (bytes.length >= 6) {
      dosHeaderFields.push(this.createField('Pages', bytes[4] | (bytes[5] << 8), 4, 2))
    }

    if (bytes.length >= 8) {
      dosHeaderFields.push(this.createField('Relocation Items', bytes[6] | (bytes[7] << 8), 6, 2))
    }
    ...
    const dosHeader = this.createField('DOS Header', null, 0, 64, dosHeaderFields)
    ...
    if (peOffset > 0 && peOffset < bytes.length - 24 && bytes.length > peOffset + 4) {
      ...
      if (peSignature === 0x00004550) {
        ...
        peHeaderFields.push(this.createField('Signature', 'PE\\0\\0 (0x00004550)', peOffset, 4))

        if (bytes.length > peOffset + 6) {
          const machine = bytes[peOffset + 4] | (bytes[peOffset + 5] << 8)
          peHeaderFields.push(
            this.createField('Machine', this.getMachineType(machine), peOffset + 4, 2)
          )
        }

        if (bytes.length > peOffset + 8) {
          const numSections = bytes[peOffset + 6] | (bytes[peOffset + 7] << 8)
          peHeaderFields.push(this.createField('Number of Sections', numSections, peOffset + 6, 2))
        }
        ...
        if (bytes.length > peOffset + 12) {
          ...
          peHeaderFields.push(this.createField('Timestamp', dateStr, peOffset + 8, 4))
        }

        const peHeader = this.createField('PE Header', null, peOffset, 24, peHeaderFields)
    ...
    while (offset < bytes.length - 30 && fileCount < maxFiles) {
      ...
      if (sig === 0x04034b50) {
        ...
        if (offset + 30 <= bytes.length) {
          ...
          entryFields.push(this.createField('Compressed Size', compSize, offset + 18, 4))

          entryFields.push(this.createField('Uncompressed Size', uncompSize, offset + 22, 4))
  ...
  parsePNG(bytes) {
    ...
    fields.push(this.createField('PNG Signature', '89 50 4E 47 0D 0A 1A 0A', 0, 8))
    ...
    while (offset < bytes.length - 12 && chunkCount < maxChunks) {
      ...
      const chunkFields = []
      chunkFields.push(this.createField('Length', chunkSize, offset, 4))

      chunkFields.push(this.createField('Type', chunkType, offset + 4, 4))
      ...
      if (chunkType === 'IHDR' && bytes.length >= offset + 12 + 13) {
        ...
        chunkFields.push(this.createField('Width', width, offset + 8, 4))

        chunkFields.push(this.createField('Height', height, offset + 12, 4))
      }
