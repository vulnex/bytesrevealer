      ...
      if (bytes.length > peOffset + 24 && peOffset > 0 && peOffset < bytes.length - 4) {
        ...
        if (peSignature === 0x00004550) {
          ...
          const _symTablePtr =
            bytes[peOffset + 12] |
            (bytes[peOffset + 13] << 8) |
            (bytes[peOffset + 14] << 16) |
            (bytes[peOffset + 15] << 24)
          const _numSymbols =
            bytes[peOffset + 16] |
            (bytes[peOffset + 17] << 8) |
            (bytes[peOffset + 18] << 16) |
            (bytes[peOffset + 19] << 24)
          ...
          if (optHeaderSize > 0 && bytes.length > peOffset + 24 + optHeaderSize) {
            ...
            if (bytes.length > optOffset + 28) {
              const entryPoint =
                bytes[optOffset + 16] |
                (bytes[optOffset + 17] << 8) |
                (bytes[optOffset + 18] << 16) |
                (bytes[optOffset + 19] << 24)
              const imageBase =
                bytes[optOffset + 28] |
                (bytes[optOffset + 29] << 8) |
                (bytes[optOffset + 30] << 16) |
                (bytes[optOffset + 31] << 24)
