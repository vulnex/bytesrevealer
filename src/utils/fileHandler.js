...
export async function processFileInChunks(file, _options = {}, onProgress) {
...
function _calculateEntropy(counts, totalBytes) {
  let entropy = 0
...
export async function calculateFileHashes(file, _onProgress) {
  const chunkSize = 1024 * 1024 // 1MB chunks
  const _totalChunks = Math.ceil(file.size / chunkSize)
      ...
      if (
        ...
        if (nfat >= 1 && nfat <= 20 && bytes.length >= 8 + nfat * 20) {
          ...
          if ([1, 6, 7, 0x01000007, 12, 0x0100000c, 0x0200000c, 18, 0x01000012].includes(cpu)) {
            logger.debug(
              'Overriding file-type class detection: Mach-O Universal Binary with',
              nfat,
              'architectures'
            )
            return {
...
function getFileTypeDescription(ext) {
  const descriptions = {
    ...
    // app: 'macOS Application', (duplicate - see app in archives section above)
